...
const getList = function(tableName) {
  ...
  return new Request(
    `https://api.airtable.com/v0/${config.base}/${tableName}?maxRecords=${
      config.maxRecords
    }`,
    {
      method: "get",
...
const getPagedListView = function(tableName, view, pageSize, offset) {
  ...
  return new Request(
    `https://api.airtable.com/v0/${
      config.base
    }/${tableName}?pageSize=${pageSize}&offset=${offset}&view=${view}`,
    {
      method: "get",
      headers: new Headers({
        Authorization: `Bearer ${config.apiKey}`
...
const findRecordsByBoolean = function(
  tableName,
  booleanFieldName,
  booleanFieldValue
) {
  ...
  return new Request(
    `https://api.airtable.com/v0/${
      config.base
    }/${tableName}?filterByFormula=(${booleanFieldName}='${booleanFieldValue}')`,
    {
      method: "get",
      headers: new Headers({
        Authorization: `Bearer ${config.apiKey}`
...
const findRecords = function(tableName, fieldName, fieldValue, view) {
  return new Request(
    `https://api.airtable.com/v0/${
      config.base
    }/${tableName}?view=${view}&filterByFormula=(FIND(LOWER("${encodeURIComponent(
...
const getChildRecords = function(tableName, values, view) {
  ...
  return new Request(
    `https://api.airtable.com/v0/${
      config.base
    }/${tableName}?view=${view}&filterByFormula=(OR(` +
      orString +
      `))`,
    {
      method: "get",
...
const createRecord = function(tableName, obj) {
  console.log("create", tableName, obj);
  return new Request(
    `https://api.airtable.com/v0/${config.base}/${tableName}`,
    {
      method: "post",
      body: JSON.stringify(obj),
      headers: new Headers({
...
const createOrUpdateRecord = async function(tableName, obj) {
  ...
  var resp = await fetch(
    new Request(
      `https://api.airtable.com/v0/${
        config.base
      }/${tableName}?filterByFormula=(FIND(LOWER("${encodeURIComponent(
        createParam(v1)
      )}"),LOWER({${k1}})))`,
      {
        method: "get",
